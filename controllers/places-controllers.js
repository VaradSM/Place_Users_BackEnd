...
const getPlaceById = async (req, res, next) => {
    ...
    if (!place){
        const error = new HttpError('Could not find place for the pid', 404);
        return next(error);
}
...
const getPlacesByUserId = async (req, res, next)=>{
    ...
    if (!userWithPlaces || userWithPlaces.places.length === 0){
        return next(new HttpError('Could not find places for the uid', 404));
    }
...
const deletePlace = async (req, res, next) => {
    ...
    if(!place) {
        const error = new HttpError('Could not find the place for this id', 400);
        return next(error);
    }
    ...
    try{
        ...
        const error = new HttpError(
            'Something went wrong and could not delete place',
            500
        );
        return next(error);
    }
